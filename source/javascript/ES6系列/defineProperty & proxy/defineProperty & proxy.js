...
// arc.num // console.log('执行了 get 操作')
...
// 此外，我们可能需要监控很多属性值的改变，要是一个一个写，也很累呐，所以我们简单写个 watch 函数。使用效果如下：
...
// Proxy
// 使用 defineProperty 只能重定义属性的读取（get）和设置（set）行为，到了ES6，提供了 Proxy，可以重定义更多的行为，比如 in 、 delete 、 函数调用等更多行为
// Proxy 这个词的原意是代理，用在这里表示由它来“代理”某些操作，ES6 原生提供 Proxy 构造函数，用来生成 Proxy 实例。我们来看它的语法

// var proxy = new Proxy(target, handler)
// proxy 对象的所有用法，都是上面这种形式，不同的只是 handler 参数的写法，其中，new Proxy（）表示生成一个 Proxy 实例，target 参数表示所要拦截的目标对象，handler
// 参数也是一个对象，用来定制拦截行为
...
// 除了 get 和 set 之外，proxy 可以拦截多达13种操作，比如has(target,propKey), 可以拦截 propKey in proxy 的操作，返回一个布尔值
...
// apply 方法可以接受三个参数，分别是目标对象、目标对象的上下文对象（this) 和 目标对象的参数数组，不过这里我们简单演示一下
// var target = function () {
//   return 'I am the target'
// }
// var handler = {
//   apply: function (tar, context, args) {
